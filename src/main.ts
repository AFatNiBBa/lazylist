...
namespace LazyList {
    ...
    export abstract class LazyAbstractList<T> {
         ...
         */
        traverse(f: Convert<T, Iterable<T>, LazyTraverseList<T>>, p?: Predicate<T, LazyTraverseList<T>>, flip?: boolean) {
            return new LazyTraverseList<T>(this, f, p, flip);
        }
    ...
    export class LazyTraverseList<T> extends LazySourceList<T, T> {
        constructor (source: Iterable<T>, public f: Convert<T, Iterable<T>, LazyTraverseList<T>>, public p?: Predicate<T, LazyTraverseList<T>>, public flip?: boolean) { super(source); }

        static *traverse<T, TList>(source: Iterable<T>, f: Convert<T, Iterable<T>, TList>, p?: Predicate<T, TList>, flip: boolean = false, list?: TList) {
            var i = 0;
    ...
    export class LazyInsertList<T> extends LazyFixedList<T, T> {
        constructor (source: Iterable<T>, public v: T, public i?: number) { super(source); }
        ...
        *[Symbol.iterator]() {
            ...
            const iter: MarkedIterator<T> = list[Symbol.iterator]();
            const i = l + this.i;
            if (i >= 0)
            {
                yield* LazyTakeList.take(iter, i);
                if (iter.done) return; // This is able to add in the end because the iterator doesn't know if it is done until it advances PAST the end
                yield this.v;
            }
    ...
    export class LazyRemoveList<T> extends LazySourceList<T, T> {
        constructor (source: Iterable<T>, public v: T, public i: number = 1) { super(source); }
    ...
    export class LazyGroupList<T, TKey, I = Grouping<T, TKey>> extends LazySourceList<I, Grouping<T, TKey>> {
        /**
        ...
        reGroup<TResult>(f: Convert<Grouping<T, TKey>, Iterable<TResult>, LazyGroupList<T, TKey, I>>): LazyGroupList<TResult, TKey> {
            return new LazyGroupList<TResult, TKey>(this.select((x, i) => new Grouping(x.key, f(x, i, this))));
        }
    }

    /** Output of {@link groupBy} */
    export class LazyGroupByList<T, TKey> extends LazyGroupList<T, TKey, T> {
    ...
    export class LazyStoreByList<T, TKey> extends LazyAbstractList<T> {
        processed: number = 0;
        cached: T[] = [];
        constructor(public key: TKey, public store: LazyStore<T, TKey>) { super(); }
        ...
        *[Symbol.iterator]() {
            ...
            for (var value: T; !({ value, done: store.done } = store.iter.next()).done; )
            {
                ...
                else store.get(k).cached.push(value);
            }
        }
    ...
    export class LazyStore<T, TKey> {
        ...
        get(k: TKey) {
            ...
            if (!out)
                this.map.set(k, out = new LazyStoreByList<T, TKey>(k, this));
            return out;
        }
    ...
    export class LazyCacheList<T> extends LazyAbstractCacheList<T, T, T[]> {
         ...
         */
        getIterator(load = 0) {
            return new BufferIterator<T>(this.getBufferFunc(load));
        }
